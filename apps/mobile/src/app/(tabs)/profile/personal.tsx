...
function Field({
  ...
  return (
    <View className="mt-[16px]">
      <Text
        className="mb-[8px] ml-[3px] text-[13px]"
        style={{ color: C.label }}
      >
        {label}
      </Text>
...
export default function PersonalDetails() {
  ...
  const [dob, setDob] = useState<Date | null>(
    fromISODate(self?.dateOfBirth ?? null),
  );
  ...
  const pickGender = () =>
    ActionSheetIOS.showActionSheetWithOptions(
      {
        options: [...GENDERS, "Cancel"],
        cancelButtonIndex: GENDERS.length,
        title: "Gender",
      },
      ...
      <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
        ...
        >
          ...
          <Host style={{ position: "absolute" }}>
            <BottomSheet
              isPresented={dobOpen}
              onIsPresentedChange={setDobOpen}
              fitToContents
            >
              <DatePicker
