...
function PlayGlyph({ size }: { size: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden>
      <path
        d="M3.6 2.3 14 12 3.6 21.7A1.7 1.7 0 0 1 3 20.4V3.6c0-.5.2-1 .6-1.3Z"
        fill="#34C79A"
      />
      <path
        d="M17.6 8.6 14 12 3.6 2.3c.4-.4 1-.4 1.5-.1l12.5 6.4Z"
        fill="#8AF5FF"
      />
      <path
        d="M17.6 15.4 5.1 21.8c-.5.3-1.1.3-1.5-.1L14 12l3.6 3.4Z"
        fill="#FF7A6B"
      />
      <path
        d="M21.4 10.9c.8.5.8 1.7 0 2.2l-3.8 2.3L14 12l3.6-3.4 3.8 2.3Z"
        fill="#FFC745"
      />
    </svg>
  );
    ...
    <>
      <div className="bg-navy px-5 py-2.5 text-center text-[13px] font-medium text-white/90">
        ...
        <span className="text-cyan-glow">
          free for patients of DentaCare Clinic.
        </span>
      </div>
    ...
    <footer className="mt-24 border-t border-hairline bg-white/60">
      <div className="mx-auto max-w-6xl px-5 py-14 sm:px-8">
        <div className="flex flex-col gap-10 sm:flex-row sm:justify-between">
          <div className="max-w-xs">
            ...
            <p className="mt-3 text-[14px] leading-relaxed text-muted">
              The patient app for DentaCare Clinic. Booking, intake, messaging
              and video consultations — without the phone call.
            </p>
          </div>
        ...
        <div className="mt-12 rounded-[var(--radius-tile)] border border-coral/25 bg-coral/[0.06] px-4 py-3 text-[13.5px] font-medium text-navy">
          Not for emergencies. If you have trouble breathing or swallowing,
          facial swelling, or bleeding that will not stop, call 911 or go to
          your nearest emergency room.
        </div>

        <p className="mt-8 text-[13px] text-muted">
          © {new Date().getFullYear()} [LEGAL ENTITY NAME]. Dentify is
          scheduling and communication software. It does not provide dental
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 pt-14 pb-4 sm:px-8 sm:pt-20">
      ...
      <div className="mt-6 flex flex-wrap gap-x-6 gap-y-1 text-[13.5px] text-muted">
        <span>
          Effective{" "}
          <strong className="font-semibold text-navy">{effective}</strong>
        </span>
        <span>
          Last updated{" "}
          <strong className="font-semibold text-navy">{updated}</strong>
        </span>
      </div>
      ...
      {draft ? (
        <div className="mt-8 rounded-[var(--radius-tile)] border border-coral/30 bg-coral/[0.07] p-5">
          ...
          <p className="mt-1.5 text-[14px] leading-relaxed text-navy/80">
            This document is a working draft written from the
            application&rsquo;s source code. It contains unresolved placeholders
            in square brackets and has not been reviewed by a lawyer. It is not
            yet binding on anyone.
          </p>
        </div>
      ) : null}
      ...
      <div className="mt-16 rounded-[var(--radius-tile)] border border-hairline bg-white p-6">
        <p className="text-[14.5px] text-muted">
          ...
          <strong className="text-navy">[SUPPORT EMAIL]</strong>.
        </p>
      </div>
    </main>
  );
