...
async function main() {
  if (!KEY || !SECRET) {
    console.error(
      "✗ NEXT_PUBLIC_STREAM_API_KEY / STREAM_API_SECRET not set in apps/web/.env",
    );
    process.exit(1);
  }
  console.log(
    `api key: ${KEY.slice(0, 4)}… (${KEY.length} chars)   secret: ${SECRET.length} chars`,
  );

  try {
    const res = await StreamChat.getInstance(KEY, SECRET).getAppSettings();
    const app = res.app as
      { name?: string; channel_configs?: Record<string, unknown> } | undefined;
    console.log(`✓ Stream reachable — app "${app?.name ?? "(unnamed)"}"`);
    console.log(
      `  channel types: ${Object.keys(app?.channel_configs ?? {}).join(", ") || "(none)"}`,
    );
    console.log(
    ...
    if (status === 401) {
      console.error(
        "  401 = the key resolves but the SECRET is wrong. Re-copy both from the",
      );
      console.error(
        "  same app in the dashboard; a key from one app and a secret from another",
      );
      console.error("  fails exactly like this.");
    } else if (status === 503) {
      console.error(
        '  503 "no healthy upstream" = auth PASSED, then Stream had no backend to',
      );
      console.error(
        "  route to. Your credentials are correct; the app itself is not serving.",
      );
      console.error(
        "  Usually a just-created app still provisioning, or a bad region.",
      );
      console.error(
        "  Wait a few minutes and re-run. If it persists, recreate the app (region",
      );
