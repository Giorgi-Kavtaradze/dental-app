...
const deleteFolder = vi.fn(
  async (f: string) => void order.push(`imagekit:${f}`),
);
...
vi.mock("@/db", () => ({
  db: {
    select: () => ({
      from: () => ({
        where: async () => [{ id: "patient-1" }, { id: "patient-2" }],
      }),
    }),
...
describe("DELETE /api/me", () => {
  ...
  it("revokes vendor assets before dropping the rows that address them", async () => {
    ...
    expect(order.indexOf("imagekit:/patient-uploads/ai/user-1")).toBeLessThan(
      db,
    );
  ...
  it("deletes the conversations before the Stream identity", async () => {
    ...
    expect(order.indexOf("stream:channels")).toBeLessThan(
      order.indexOf("stream:user"),
    );
  });
  ...
  it("clears both private photo folders, not just the assistant one", async () => {
    ...
    expect(deleteFolder).toHaveBeenCalledWith(
      "/patient-uploads/appointments/user-1",
    );
  });
