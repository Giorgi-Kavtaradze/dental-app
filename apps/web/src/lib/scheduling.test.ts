...
describe("overlaps", () => {
  ...
  it("treats intervals as half-open — touching endpoints do not overlap", () => {
    expect(
      overlaps(
        t("2025-06-16T13:00:00Z"),
        t("2025-06-16T13:30:00Z"),
        t("2025-06-16T13:30:00Z"),
        t("2025-06-16T14:00:00Z"),
      ),
    ).toBe(false);
  });
...
describe("availableSlots — a normal day", () => {
  ...
  it("aggregates across dentists and sorts by time", () => {
    ...
    expect(new Set(slots.slice(0, 2).map((s) => s.dentistId))).toEqual(
      new Set([D, OTHER]),
    );
  });
});
...
describe("availableSlots — DST", () => {
  ...
  it("spans a spring-forward weekend without dropping or duplicating a day", () => {
    ...
    expect(slots).toHaveLength(62);
    expect(new Set(slots.map((s) => formatClinicTime(s.startsAt))).size).toBe(
      31,
    );
  });
...
describe("classifySlot — the write-path guard", () => {
  ...
  it("accepts a slot the availability endpoint would offer", () => {
    expect(
      classifySlot({ ...base, startsAt: at("2025-06-16T13:00:00Z") }),
    ).toBe("ok");
  });
  ...
  it("rejects a time outside working hours", () => {
    ...
    expect(
      classifySlot({ ...base, startsAt: at("2025-06-16T07:00:00Z") }),
    ).toBe("invalid");
  });

  it("rejects a start that is off the 15-minute grid", () => {
    expect(
      classifySlot({ ...base, startsAt: at("2025-06-16T13:07:00Z") }),
    ).toBe("invalid");
  });
  ...
  it("rejects a slot that would run past closing", () => {
    expect(
      classifySlot({
        ...base,
        durationMinutes: 90,
        startsAt: at("2025-06-16T19:45:00Z"),
      }),
    ).toBe("invalid");
  });
  ...
  it("rejects a slot inside the 2-hour lead time", () => {
    ...
    expect(
      classifySlot({ ...base, now, startsAt: at("2025-06-16T13:00:00Z") }),
    ).toBe("invalid");
  });

  it("rejects a day the dentist does not work", () => {
    expect(
      classifySlot({ ...base, startsAt: at("2025-06-15T13:00:00Z") }),
    ).toBe("invalid"); // Sunday
  });
  ...
  it("rejects another dentist's slot", () => {
    expect(
      classifySlot({
        ...base,
        dentistId: OTHER,
        startsAt: at("2025-06-16T13:00:00Z"),
      }),
    ).toBe("invalid");
  });
    ...
    const busy: BusyInterval[] = [
      {
        dentistId: D,
        startsAt: at("2025-06-16T13:00:00Z"),
        endsAt: at("2025-06-16T13:30:00Z"),
      },
    ];
    expect(
      classifySlot({ ...base, busy, startsAt: at("2025-06-16T13:00:00Z") }),
    ).toBe("taken");
  });
  ...
  it("reports a slot blocked by time off as taken", () => {
    const busy: BusyInterval[] = [
      {
        dentistId: D,
        startsAt: at("2025-06-16T16:00:00Z"),
        endsAt: at("2025-06-16T17:00:00Z"),
      },
    ];
    expect(
      classifySlot({ ...base, busy, startsAt: at("2025-06-16T16:00:00Z") }),
    ).toBe("taken");
  });
