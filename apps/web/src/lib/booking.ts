...
export async function loadSchedulingInputs(
  ...
  const rangeEnd = new Date(
    clinicInstant(to, 23, 59).getTime() + 24 * 3600_000,
  );
  ...
  const [hours, booked, off] = await Promise.all([
    db
      .select()
      .from(workingHours)
      .where(inArray(workingHours.dentistId, dentistIds)),
    db
      .select({
...
export async function assertSlotBookable({
  ...
  const [offers] = await db
    ...
    .where(
      and(
        eq(dentistServices.dentistId, dentistId),
  ...
  if (verdict === "invalid") {
    throw badRequest(
      "That time is not available. Please pick one of the offered slots.",
    );
  }
}
